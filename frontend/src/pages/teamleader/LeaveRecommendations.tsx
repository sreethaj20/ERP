...
export default function LeaveRecommendations() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            setPendingLeaves(teamLeaves.filter((l: any) => l.status?.toLowerCase() === 'pending'));
    ...
    const statusColor = (status: string) => {
        ...
        if (s === 'approved') return { color: '#30d158', bg: 'rgba(48,209,88,0.12)' };
        if (s === 'rejected') return { color: '#ff453a', bg: 'rgba(255,69,58,0.12)' };
        ...
        <div className="dashboard-container">
            ...
            <div style={{ display: 'flex', gap: '16px', marginBottom: '30px' }}>
                {[
                    ...
                    { label: 'Approved', value: allTeamLeaves.filter(l => l.status?.toLowerCase() === 'approved').length, color: '#30d158' },
                    { label: 'Rejected', value: allTeamLeaves.filter(l => l.status?.toLowerCase() === 'rejected').length, color: '#ff453a' },
                ].map(stat => (
                ...
                <GlassCard title="⏳ Pending Your Approval" subtitle="Leave requests from your direct reports waiting for your review">
                    <div style={{ maxHeight: "500px", overflowY: "auto", marginTop: "15px" }}>
                        {pendingLeaves.length === 0 ? (
                            ...
                            const empName = leave.employee_name || leave.name || leave.employee_id || "Employee";
                            return (
                                <div
                                    key={leave.id}
                                    style={{
                                ...
                                >
                                    <div style={{ display: 'flex', gap: '15px', alignItems: 'center', flex: 1 }}>
                                        ...
                                        <div>
                                            <div style={{ fontWeight: '600', color: 'var(--text-primary)', fontSize: '15px' }}>{empName}</div>
                                    ...
                                    <div style={{ display: 'flex', gap: '10px' }}>
                                        <button onClick={() => handleAction(leave.leave_id || leave.id, 'approve')} className="apple-btn" style={{ padding: '8px 18px', fontSize: '12px', background: 'rgba(48,209,88,0.12)', color: '#30d158', border: '1px solid rgba(48,209,88,0.2)', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                            <FaCheck size={11} /> Approve
                                        </button>
                                        <button onClick={() => handleAction(leave.leave_id || leave.id, 'reject')} className="apple-btn" style={{ padding: '8px 18px', fontSize: '12px', background: 'rgba(255,69,58,0.12)', color: '#ff453a', border: '1px solid rgba(255,69,58,0.2)', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                    ...
                                    {allTeamLeaves.map((leave, i) => {
                                        ...
                                        return (
                                            <tr key={i} style={{ borderBottom: '1px solid var(--border-light)' }}>
                                                <td style={{ padding: '12px', fontWeight: '600', color: 'var(--text-primary)' }}>{empName}</td>
